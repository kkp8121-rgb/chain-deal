...
- **현재 드리프트 상태**: `run-sim.cjs` 만 부적 **13종**(+위치-맥락 3종 다리/오름계단/주춧돌, `handBonus`에 boss 인자 미러) + `fiveKind` 족보 + **골드 경제** + **난이도 사다리(STK)**까지 동기화됨. **`balance-check.cjs` · `strategy-sim.cjs` · `hand-frequency.cjs` 는 구 5부적 기준이고 `fiveKind` 미반영** — 이들로 신규 부적/족보를 검증하려면 먼저 동기화할 것.
