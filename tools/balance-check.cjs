...
//
// ★Phase 0 Step 4b: connect/climbSealed·evalHand·HAND_BONUS·blindBase/sparkComp는 더 이상 여기서
//   재정의하지 않고 src/ 를 require한다(grep=1 대상). 검증: 이 파일의 32장 무중복 덱(맨덱, enh=wild만)에서는
//   rank 그룹 최대치가 4를 못 넘어 evalHand의 fiveKind 분기가 도달 불가 — 교체는 수치상 무변경(단일라운드
//   문법게이트 역할은 그대로, gain()의 charm-free 단순 점수 계산 본체는 부적 없는 이 파일 성격상 유지).
const fs = require("fs");
const path = require("path");
const { connect, climbSealed } = require("../src/rules/connect.cjs");
const { evalHand } = require("../src/rules/hands.cjs");
const { HAND_BONUS } = require("../src/content/hands.cjs");
const { blindBase, sparkComp } = require("../src/rules/blinds.cjs");
// content/bosses.cjs는 t()를 별도 require 없이 참조(build.mjs 텍스트 스플라이스 관례) → run-sim.cjs:39-40과
// 동일 패턴으로 전역에 걸어 해결.
global.t = require("../src/content/locale/i18n.cjs").t;
const { BOSSES } = require("../src/content/bosses.cjs");
const HAND_OF = id => id === "stingy" ? 2 : 3;   // 정본엔 hand 필드 없음(단일라운드 손패수는 이 툴 전용) → 로컬 유지
...
function gain(row, card, boss) {   // 맨 덱(enh 없음) 단일라운드 → rust는 여기선 무효(enh 의존, run-sim에서 검증). 부적 없음(charm-free 문법게이트) → scoreCard 어댑터화 안 함.
  row.push(card);
  ...
  if (left && connect(card, left, boss) && !climbSealed(card, left, boss) && !(boss === "frost" && row.length <= 2)) {   // 냉각: 첫 2장 무연결 / 내리막: 오름 ±1 봉인
    for (let i = row.length - 1; i > 0; i--) { if (connect(row[i], row[i - 1], boss) && !climbSealed(row[i], row[i - 1], boss)) rl++; else break; }
    let mult = rl - 1;
...
for (const b of BOSSES) {
  ...
  const hand = HAND_OF(b.id);
  console.log(`  ${b.actBoss ? "👑" : "  "} ${b.icon} ${b.name} (A${b.act}, t=${t}, 손패${hand}): ${clearRate(b.id, hand, t, 1, true)}%`);
}
