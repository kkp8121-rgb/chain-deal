...
function starterDeck(){ const d=[]; for(let s=0;s<4;s++) for(let r=1;r<=8;r++) d.push({suit:s,rank:r,enh:null}); const ord=d.map((c,i)=>[c.rank,i]).sort((a,b)=>a[0]-b[0]); for(let k=0;k<4;k++) d[ord[k][1]].enh='wild'; return d; }   // 불씨덱(v3.29): 최저랭크 4장 wild(연결밀도↑=캐주얼 도파민). sparkComp 보정과 세트(blindBase 150 불변) — index fullDeck 미러
...
let onCardScored = null;   // gate.cjs가 G1(mult 캡 계측)용으로 주입 — 기본 null=no-op(RNG/setRNG와 동일 패턴, 기존 동작 무회귀)
function setCardHook(fn){ onCardScored = fn || null; }
function gain(row, card, boss, owned, deckSize){
  const left = row[row.length-1];
  row.push(card);
  const r = scoreCard(row, card, left, scoreCtxSim(boss, owned, deckSize));
  if (onCardScored) onCardScored(r, boss);   // ★gate.cjs(G1b)가 boss도 받아 기대 캡(닻=3/그외=25)을 판정 — 인자 추가는 기존 훅(1-인자) 무회귀(여분 인자 무시)
  return r.gained;
}
...
// stats(선택, gate.cjs G1c 전용): {exhausted}에 draw() fallback(카드더미 소진→랜덤 재추첨) 발동 횟수를 누적.
// 정상 플레이(핸드N+8 ≤ ds, thin 하한 20이 보장)면 절대 발동 안 함 — 발동 시 "덱 고갈"(HANDOVER §3.2 손패회수
// 버그류) 재발 신호. 미지정 시 완전 no-op(기존 6개 호출부 무회귀).
function playRound(deck, owned, boss, handN, ante, pick, stats){
  pick = pick || defaultPick;
  const dk=shuffle(deck.slice()); let di=0; const ds=deck.length;
  const draw=()=>{ if(di<dk.length) return dk[di++]; if(stats) stats.exhausted=(stats.exhausted||0)+1; return dk[ri(dk.length)]; };
...
// ★gate.cjs(C1) 전용 — STK/DMULT는 runFull() 호출부 밖에서 읽는 코드(예: tools/funqa/runner.cjs의
//   runFullInstrumented가 R.blindTarget/R.pickBoss를 runFull 경유 없이 직접 호출)가 있어, runFull(...,variant:"high"...)
//   호출 뒤 이 모듈 레벨 변수가 잔류하면 그 직후 funqa 계측이 오염된다(실측: gate.cjs G3의 high덱 parity 체크 뒤
//   G6/G7을 그대로 돌리면 DMULT=1.29가 새어들어가 대중재미가 거짓으로 하락). CLI 블록·기존 runFull/simulateStrat
//   호출부는 그대로(무회귀) — gate.cjs가 orchestration 경계에서 명시적으로 호출.
function resetSimState(){ STK=0; DMULT=1; }
// ★gate.cjs(C1a) 모듈화: 5번째 opts는 선택 인자 — 기존 4-인자 호출(strat,acc,stake,variant, 아래 CLI 블록 포함)은
//   opts=undefined→{}로 폴백되어 완전히 동일하게 동작(무회귀). opts.forceOwned=시작 보유 부적 강제 주입(G3 EV z
//   forced-inclusion용) / opts.onStep({phase:'round'|'shop',...deckBefore,deckAfter})=라운드/상점 전후 덱 크기 계측
//   콜백(G1 카드보존 불변식용) — 둘 다 미지정 시 아무 것도 안 함(오케스트레이션 로직·수치 변경 0).
function runFull(strat, acc, stake, variant, opts){   // variant: 시작덱 id(미지정=standard)
  opts = opts || {};
  STK = stake|0;
  const v = DECKS.find(d=>d.id===variant) || DECKS[0];
  DMULT = v.dmult;
  const state={deck:v.build(), owned:(opts.forceOwned||[]).slice(), bonusHand:0, gold:0};
  for(let ante=1;ante<=8;ante++){
    ...
    for(let blind=0;blind<=2;blind++){
      ...
      const deckBeforeRound = state.deck.length;
      const roundStats = opts.onStep ? {} : null;   // G1(c) 전용 계측(exhausted) — onStep 미지정이면 할당 0(무회귀)
      const sc=playRound(state.deck, state.owned, boss?boss.id:null, handN, ante, undefined, roundStats);
      if(opts.onStep) opts.onStep({phase:'round', ante, blind, deckBefore:deckBeforeRound, deckAfter:state.deck.length, exhausted:(roundStats&&roundStats.exhausted)||0});
      ...
      const deckBeforeShop = state.deck.length;
      applyShop(state, strat);
      if(opts.onStep) opts.onStep({phase:'shop', ante, blind, deckBefore:deckBeforeShop, deckAfter:state.deck.length});
    }
  }
  return {result:"win"};
}

// ★gate.cjs(C1a) 전용 — N판 집계를 함수 호출 하나로. runFull의 acc(reach/pass/bReach/bPass)를 내부에서
//   구성해 clearRate/perBlindCond("ante-blind"→통과율)/bossCond(보스id→통과율, bossMinReach 미만 표본 제외)/
//   deathDist("ante-blind"→죽은 횟수)로 정리해 반환. run-sim CLI 콘솔 출력 로직과 완전히 분리된 순수 집계
//   함수(신규 추가 — 기존 CLI 블록은 이 함수를 쓰지 않고 그대로 유지되어 출력 무회귀).
function simulateStrat(strat, opts){
  opts = opts || {};
  const N = opts.N || 20000;
  const stake = opts.stake || 0;
  const variant = opts.variant;
  const bossMinReach = opts.bossMinReach != null ? opts.bossMinReach : 200;
  const runOpts = opts.forceOwned ? { forceOwned: opts.forceOwned } : undefined;
  const acc = { reach:{}, pass:{}, bReach:{}, bPass:{} };
  const deathDist = {};
  let win = 0;
  for(let i=0;i<N;i++){
    const r = runFull(strat, acc, stake, variant, runOpts);
    if(r.result === "win") win++;
    else { const k = `${r.ante}-${r.blind}`; deathDist[k] = (deathDist[k]||0)+1; }
  }
  const perBlindCond = {};
  for(const k in acc.reach) perBlindCond[k] = acc.reach[k] ? acc.pass[k]/acc.reach[k] : null;
  const bossCond = {};
  for(const id in acc.bReach) if(acc.bReach[id] >= bossMinReach) bossCond[id] = acc.bPass[id]/acc.bReach[id];
  return { clearRate: win/N, perBlindCond, bossCond, deathDist, N, win, reach:acc.reach, pass:acc.pass, bReach:acc.bReach, bPass:acc.bPass };
}
...
module.exports = {
  ...
  DECKS, runFull, simulateStrat, setCardHook,   // ★C1a(gate.cjs) 추가 export — 로직 변경 0, 함수 추출/훅 주입만
  resetSimState,   // ★C1b(gate.cjs) — G3(high덱 parity) 뒤 G6/G7(funqa) 오염 방지용 명시적 상태 리셋
};
