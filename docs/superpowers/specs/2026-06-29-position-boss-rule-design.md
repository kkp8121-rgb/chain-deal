...
**효과**: 오름차순 ±1 *체인 배율*을 봉인 → "오름 본능" 플레이를 카운터(핵심 ±1 수읽기를 내림·같은무늬·같은숫자로 우회). ★단 stair *부적 보너스*(maxAscLen)·bridge/keystone은 무영향 — 봉인되는 건 오름 ±1 체인 배율뿐. seal_run보다 결적(내림 ±1 허용)이라 tmult 상향. ★**사후검증(run-sim 표적 대시보드)**: stair(spatial) 빌드 표적 적중 **미입증** — 밸런스 90% vs stair 85%(델타 +5pp)이나 act2 동료(인색한손·연결세 +5pp)와 동급 = 그리디가 봉인을 자동 우회. "직격"은 우회 안 하는 나이브 클라이머에 한정.
