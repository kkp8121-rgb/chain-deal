...
> ★**렌더러 결정(2026-07-06, 적대검증 확정)**: 발라트로급 juice는 이 아트 단계에서 **게임 보드만 DOM→WebGL(PixiJS, esbuild 번들)로 스왑**해 확보(셰이더·스프링물리·GPU 파티클·60fps). 메뉴/HUD/설정은 DOM 유지. **rules/content/sim/gate 무손상**(grep 검증 → 밸런스 단일소스 0비용 보존). 절차적 아이콘이 param→그리기로 재저작되는 이 시점이 canvas/WebGL 타깃의 최저 증분 지점. **Godot 전환은 기각**(툴링 bit-exact 재검증 리스크·즉시-웹-데모 상실·~반년 비출시; 22–28주 vs WebGL 8–13주). Godot는 "Deck-Verified 필수" 또는 "저사양 60fps 필수"가 하드 요구 + 콘텐츠 락 이후일 때만(그때도 Godot+C#). 상세 memory `engine-renderer-decision`.
...
### Wrap decision
- **Steam → Tauri** (권장, 2026-07-06 정정) **+ `steamworks-rs`** (Rust 바인딩). ~~구 결정: Electron+`steamworks.js`~~ — 재검토 결과 **Electron이 Steam Deck "Verified"를 위협**(Steam Linux 런타임 **libcups 부재로 Chromium 실행 실패**, `--no-sandbox` 우회 필요 + Electron 업그레이드 시 게임패드 인식 회귀) + Chromium 80–150MB 블로트/낙인. Tauri 기각 사유였던 "Node 컨텍스트 없어 `steamworks.js` 불가"는 **Rust `steamworks-rs`로 해소**(achievements/cloud/overlay/Input). 트레이드오프 = per-OS WebView(WebView2/WebKit) 검증 + Steamworks가 npm 대신 Rust. Electron은 폴백. Phase B서 최종 검증. **~3–5 days.**
...
| **Full-DOM `render()` jank on low-end Android** | Low–Med | ★근본 해법 = 아트 단계의 **보드 WebGL(PixiJS) 스왑**(GPU 배칭, 발라트로급 juice와 동시 확보 — 렌더러 결정 2026-07-06). 그 전까지는 Step 5 engine/UI split의 targeted render + 저사양 QA. 단 저사양 OEM WebView의 WebGL/파티클 성능은 **미검증 리스크로 추적**(WebGL 판정의 최강 반대축). |
