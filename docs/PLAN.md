...
> **📍 중장기 로드맵(웹리서치 2026-06-25) = HANDOVER §7 🧭.** ✅**NEAR(폴리시)**(v3.18~21) + ✅**MID 1 난이도 사다리(v3.22)** + ✅**MID 2 위치-맥락 부적(v3.23)** + ✅**MID 3 부적 시너지 확장 23종(v3.24)** + ✅**MID 4 상점 희석 완화(v3.25)** + ✅**MID 5 위치-맥락 보스룰 내리막(v3.26)** + ✅**색 settle 페이오프 투톤(v3.27, 부적 24종)** + ✅**시작덱 변형 MVP 고랭크덱(v3.28)** + ✅**불씨덱 캐주얼 도파민 수정(v3.29, 재미QA 대응 — 대중재미 60→80% PASS)**. ~~▶다음 = 상업화 로드맵 C2(부적 24→50)~~ → **C2/밸런스 완전 동결 + ▶현재 국면 = 때깔(아트) 마일스톤** (2026-07-14 방향 결정 — sim 루프 소진(트라이애그 5레버 기각)으로 검증이 인간 플레이테스트로 이관, 근거 `docs/superpowers/specs/2026-07-14-art-m1-procedural-pixel-design.md` §0). ✅**아트 M1 절차적 픽셀**(2026-07-14: 16색 팔레트·카드 32종 canvas·부적 엠블럼 22종·컨택트시트 `?art=sheet`+CVD 3종·gate 9/9 GREEN 불변) → **▶다음 = 아트 M2(보드 PixiJS 스왑+발라트로급 juice)** → M3(배경/UI킷/셸). (~~스릴러 블로커~~ = 봇/메트릭 아티팩트로 **기각·종료**, memory `thriller-artifact-verdict`.)
