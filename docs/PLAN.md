...
- 🔶 **A2. 리더보드 (2종)** — ① **데일리**(오늘 시드 top10) ② **전체/올타임**(최고점 top10) · *게임 코드 완료, Apps Script 재배포 대기*
  - 근거: 데일리는 데일리 챌린지 완성, 전체는 장기 경쟁 동기. JSONP로 시트 읽기(doGet) + 닉 입력. **남은 것: 사용자가 `playlog-appsscript.gs` 재배포(배포관리→편집, URL 유지)**
