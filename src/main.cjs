...
const DECKS=[
  ...
  {id:"high",     name:"고랭크덱", desc:"A·2 없음·7·8 2배 — 고점수·정점 빌드, 단 사치세 보스 직격", build:highDeck, dmult:1.29},
];
...
const BOSSES=[
  ...
  {id:"peasant",  icon:"🥖", name:"보릿고개",        desc:"A·2·3 카드 기본 점수 0",    tmult:0.82, act:1, actBoss:false},
  {id:"tax",      icon:"👑", name:"사치세",          desc:"7·8 카드 기본 점수 0",      tmult:0.8,  act:1, actBoss:true},
  {id:"seal_climb",icon:"⤵", name:"내리막",           desc:"오르막(↑) ±1 연결 무효 — 내림·같은무늬·같은숫자만", tmult:0.72, act:2, actBoss:false},
  {id:"stingy",   icon:"✋", name:"인색한 손",       desc:"손패가 1장 줄어듦",         tmult:0.58, act:2, actBoss:false},
  {id:"toll",     icon:"💸", name:"연결세",          desc:"체인 보너스 절반",          tmult:0.54, act:2, actBoss:false},
  {id:"rust",     icon:"🦠", name:"부식",            desc:"enh(★◆●) 효과 전부 무효",   tmult:0.6,  act:2, actBoss:true},
  {id:"seal_suit",icon:"🔒", name:"봉인된 무늬",     desc:"♠ 카드는 연결 불가",        tmult:0.47, act:3, actBoss:false},
  {id:"frost",    icon:"❄",  name:"냉각",            desc:"줄 첫 2장 연결 무효",       tmult:0.48, act:3, actBoss:false},
  {id:"mono",     icon:"🎭", name:"단일강요",        desc:"같은 숫자·연속 무효(같은 무늬만)", tmult:0.4,  act:3, actBoss:false},
  {id:"anchor",   icon:"⚓", name:"닻",              desc:"체인 배율 최대 3",          tmult:0.44, act:3, actBoss:true},
];
...
const HAND_KO={highCard:"하이카드",pair:"페어",twoPair:"투페어",trips:"트리플",straight:"스트레이트",flush:"플러시",fullHouse:"풀하우스",fourKind:"포카드",straightFlush:"스트레이트 플러시",fiveKind:"🃏 파이브 카드"};
...
function renderStats(){ const s=getStats(), el=document.getElementById("stats"); if(!el) return;
  const daily = (S&&S.daily) ? `<span style="color:var(--gold)">🗓 데일리 #${S.seed}</span> · ` : "";
  el.innerHTML=`${daily}🏆 최고 <b>${s.bestAnte?`안테 ${s.bestAnte}`:"—"}</b> · ${s.plays||0}판 <b>${s.wins||0}승</b> · 최고점 <b>${(s.bestScore||0).toLocaleString()}</b> <span style="color:#9b6fd4">›</span>`; }
function statsHTML(){ const s=getStats(), wr=s.plays?Math.round((s.wins||0)/s.plays*100):0;
  return `<h3>🏆 내 기록</h3>
  <div class="hrow"><span>최고 도달</span><b class="hname">${s.bestAnte?`안테 ${s.bestAnte}`:"—"}</b></div>
  <div class="hrow"><span>최고 점수 (한 라운드)</span><b class="hname">${(s.bestScore||0).toLocaleString()}</b></div>
  <div class="hrow"><span>플레이 / 승리</span><b class="hname">${s.plays||0}판 / ${s.wins||0}승</b></div>
  <div class="hrow"><span>승률</span><b class="hname">${wr}%</b></div>
  <p class="drawerNote">💡 안테 8 보스를 격파하면 <b style="color:var(--gold)">승리</b>! 기록을 갱신해보세요.</p>`; }
const BLINDNAME=["작은 블라인드","큰 블라인드","보스 블라인드"];

/* ---------- 상태 ---------- */
const SLOTS=8, ANTES=8, MAX_STAKE=5;   // 난이도 사다리: 0(평지)~5. run-sim STK_T/STK_AC와 길이 일치
const STAKE_NAMES=["평지","백금","인색","강철","서리","심연"], STAKE_DESC=["기본 난이도","목표 +3%","+ 통과 골드 없음","+ 후반 안테 가속","+ 보스전 손패 2장","+ 목표 추가 가속"];   // 각 티어 시그니처 규칙(누적)
...
const CHARMS=[
  ...
  {id:"noir",     name:"흑심",   desc:"검정(♠♣) 카드로 연결 시 배율 +2 (발화의 거울 · 2색 빌드)"},
  {id:"broker",   name:"중개상", desc:"정산 시 페어·투페어·트리플 족보 보너스 대폭 상승 (페어도 점수)"},
  {id:"twins",    name:"쌍둥이", desc:"겹치는 숫자 그룹(2장↑)마다 정산 보너스 +3% (최대 4그룹)"},
  {id:"compactor",name:"정련가", desc:"덱이 작을수록 모든 카드 기본 점수↑ (1장 압축당 +1, 최대 +8)"},
  {id:"runts",    name:"잔챙이", desc:"A·2·3 저랭크 카드 기본 점수 +4"},
  {id:"bridge",   name:"다리",   desc:"양옆 모두와 연결되는 카드(징검다리)마다 정산 보너스 +3% (최대 3장)"},
  {id:"stair",    name:"오름계단",desc:"줄이 왼→오 오름차순인 구간이 길수록 정산 보너스 (3칸+부터)"},
  {id:"keystone", name:"주춧돌", desc:"양 끝(맨 왼·맨 오)과 한가운데 칸 카드의 숫자가 클수록 정산 보너스 (자리값)"},
  {id:"lapidary", name:"세공사", desc:"강화 카드(★●◆)를 깔면 기본 점수 +3 (부식 보스 중엔 무효)", cost:5, cluster:"gem"},
  {id:"prism",    name:"프리즘", desc:"줄에 와일드·황금·배율석이 모두 있으면 정산 보너스 +12% (부식 보스 중엔 무효)", cost:5, cluster:"gem"},
  {id:"jewelbox", name:"보석함", desc:"정산 시 줄에 깔린 강화 카드(★●◆) 1장마다 보너스 +2.5% (최대 6장)", cluster:"gem"},
  {id:"highmult", name:"위세", desc:"7·8 고카드로 연결하면 배율 +1", cost:5, cluster:"apex"},
  {id:"magnate",  name:"거물", desc:"정산 시 줄의 7·8 고카드 1장마다 보너스 +3% (최대 5장)", cluster:"apex"},
  {id:"echo",   name:"메아리", desc:"같은 숫자로 연결하면 배율 +1", cost:3, cluster:"cartel"},
  {id:"loaded", name:"사기패", desc:"정산 시 포카드면 보너스 +10%, 파이브카드(같은숫자 5장)면 +30%", cluster:"cartel"},
  {id:"climax", name:"절정",   desc:"풀하우스 이상 족보를 만든 라운드, 줄의 최장 연결이 6칸부터 길수록 정산 보너스 추가 (최대 8연결)", cluster:"cartel"},
  {id:"evenodd",   name:"홀짝 정렬", desc:"정산 시 줄이 짝수 또는 홀수 한쪽으로 6장부터 치우칠수록 보너스 (초과분마다 +4%)", cluster:"parity"},
  {id:"paritybet", name:"패리티 도박", desc:"줄 전체가 짝수만 또는 홀수만이면 정산 보너스 +30% (와일드는 자유)", cluster:"parity"},
...
const UNLOCKS={
  ...
  bridge:   {cond:(st,row)=> bridgeCount(row)>=3,                hint:"한 줄에 양옆 연결 카드 3장"},
  stair:    {cond:(st,row)=> maxAscLen(row)>=5,                  hint:"한 줄에 5칸 연속 오름차순"},
  keystone: {cond:(st,row)=> row.length>=8 && row[0].rank>=7 && row[3].rank>=7 && row[7].rank>=7, hint:"0·3·7번 칸에 7+ 배치"},
...
function charmsHTML(){
  ...
  for(const c of CHARMS){
    ...
    else h+=`<div class="hrow" style="opacity:.55"><span><b class="hname">🔒 ？？？</b><br><span class="hdesc">해금 조건: ${(UNLOCKS[c.id]||{}).hint||"비공개"}</span></span><span class="hsmall">잠김</span></div>`;   // ★불변식: 시작 부적 아닌 모든 부적은 UNLOCKS 항목 필요(없으면 영구 잠김)
  }
...
function settle(){
  ...
  document.getElementById("tTarget").textContent=`목표 ${S.target.toLocaleString()}`;
  ...
  if(pass){
    ...
    if(wasBoss && S.ante>=ANTES){ btn.textContent="🏆 승리!"; _tallyNext=victory; }
    else { btn.textContent="다음 → 상점"; _tallyNext=openShop; }
  } else {
    S.over=true;
    res.textContent="💀 패배 — 목표 미달"; res.className="tallyResult fail";
...
function victory(){
  ...
  { const m=getMeta(); const nx=Math.min(MAX_STAKE,(S.stake||0)+1); if(nx>m.maxStake){ m.maxStake=nx; saveMeta(m);   // 스테이크 N 클리어 → N+1 해금
      setTimeout(()=>{ banner(`🔓 난이도 St${nx} ${STAKE_NAMES[nx]} 해금! (${STAKE_DESC[nx]})`, "var(--gold)"); try{beep(720,.08); setTimeout(()=>beep(960,.1),100);}catch(e){} }, 1500); } }
...
function pickDeckCard(p){
  ...
  body.innerHTML=`<div style="font-size:13px;color:#aab3d6">${p.type==="thin"?"제거할":p.type==="copy"?"복제할":"효과를 줄"} 카드를 고르세요</div>`;
...
function render(){
  ...
  document.getElementById("deckinfo").innerHTML=`덱 ${S.deck.length}장 · 버림 ${S.discard.length}장 <span style="color:#9b6fd4;text-decoration:underline">🃏 덱 보기</span>`;
  // 안테 진행 점
  ...
  const hint=` <span class="hbhint">👆 족보표</span>`;
  if(S.row.length===0){ hbEl.innerHTML=`<span class="hbdim">🎴 족보 보너스 — 같은 무늬·숫자를 모으면 정산 때 +추가</span>`+hint; }
  else { const hk=evalHand(S.row), hb=handBonus(S.row);
    if(hb>0) hbEl.innerHTML=`🎴 <span class="hblbl">족보 보너스</span> <span class="hbname">${HAND_KO[hk]}</span> <span class="hbplus">+${hb}</span> <span class="hbdim">(체인에 추가)</span>`+hint;
    else hbEl.innerHTML=`<span class="hbdim">🎴 족보 보너스 — 현재 ${HAND_KO[hk]}, 보너스 없음</span>`+hint; }
...
const RULES_HTML=`
  <h3>🃏 CHAIN DEAL — 룰</h3>
  <p class="rtext">손패 <b>3장 중 하나</b>를 골라 줄에 깐다. 바로 <b>왼쪽 카드와 '통하면' 체인</b>이 터진다!</p>
  <div class="seg">연결되는 3가지</div>
  <div class="rrow"><div class="rlabel">① 같은 무늬</div>${mc("5","♠")}${mc("8","♠")} → 둘 다 ♠ (무늬 일치)</div>
  <div class="rrow"><div class="rlabel">② 같은 숫자</div>${mc("7","♥")}${mc("7","♦")} → 둘 다 7 (숫자 일치)</div>
  <div class="rrow"><div class="rlabel">③ 연속 숫자 (±1)</div>${mc("4","♣")}${mc("5","♠")} → 4 다음 5 (연속)</div>
  <p class="rtext">체인이 <b>길게 이어질수록 배율이 폭발</b> → 점수 ↑. 예: ${mc("3","♠")}${mc("4","♠")}${mc("5","♠")}${mc("6","♠")} 처럼 길게!</p>
  <p class="rtext">8장을 다 깔아 <b>목표 점수</b>를 넘기면 통과.</p>
  <div class="seg">진행 / 승리</div>
  <p class="rtext">안테 1~8, 각 안테 = <b>작은 → 큰 → 보스</b> 블라인드. 보스는 특수 규칙으로 방해한다. <b>안테 8 보스 격파 = 승리!</b> 목표 미달 = 처음부터 다시.</p>
  <div class="seg">+ 족보 보너스</div>
  <p class="rtext">8장으로 포커 족보가 되면 정산 때 <b style="color:var(--gold)">추가 점수</b>. (족보 보너스 문구를 누르면 족보표가 열려요)</p>`;
...
function deckHTML(){
  ...
  let h=`<h3>🃏 현재 덱 (${all.length}장)</h3><p class="drawerSub">덱 + 버림 + 손패 + 줄 전체. 빌드 확인·이상 감시용. (★와일드 ◆배율 ●황금)</p>`;
  for(let s=0;s<4;s++){ const cs=all.filter(c=>c.suit===s); if(!cs.length) continue;
    h+=`<div class="seg">${suitG(s)} ${cs.length}장</div><div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:4px">${cs.map(mcEnh).join("")}</div>`; }
  const rc={}; all.forEach(c=>rc[c.rank]=(rc[c.rank]||0)+1);
  const dup=Object.entries(rc).filter(([r,n])=>n>=3).map(([r,n])=>`${rankStr(+r)}×${n}`);
  h+= dup.length ? `<p class="drawerNote">💡 같은 숫자 3장+: ${dup.join(", ")} (포카드·풀하우스 빌드)</p>` : `<p class="drawerNote">같은 숫자 3장 이상 없음 — 정상 분포</p>`;
  return h;
}
...
function shareText(r){
  ...
  const sd=""+r.seed, tag = (r.daily ? `데일리 ${sd.slice(4,6)}/${sd.slice(6,8)}` : "프리런") + (r.stake ? ` · St${r.stake} ${STAKE_NAMES[r.stake]}` : "");
  const head = r.won ? `🏆 안테 ${ANTES} 클리어!` : `안테 ${r.ante}/${ANTES} 💀`;
  return `CHAIN DEAL 🔗 ${tag}\n${head} · 최고 ${r.best.toLocaleString()}\n${strip}\n${url}`;
}
function shareResult(btn){
  ...
  const done=()=>{ if(btn){ const o=btn.textContent; btn.textContent="✓ 복사됨!"; setTimeout(()=>{ btn.textContent=o; },1400); } };
...
function renderStakeLbl(){ const el=document.getElementById("stakeLbl"); if(!el) return; const mx=getMeta().maxStake; if(selStake>mx) selStake=mx;
  ...
  const d=document.getElementById("stakeDesc"); if(d) d.textContent = STAKE_DESC[selStake] + (selStake>=mx ? (mx<MAX_STAKE?"  ·  클리어 시 다음 해금":"  ·  최고 난이도") : ""); }

/* ---------- 리더보드 (A2) — 데일리 + 전체 ---------- */
function nickOr(){ return localStorage.getItem("cd_nick")||"익명"; }
function setNick(){ const cur=nickOr()==="익명"?"":nickOr(); const n=((prompt("리더보드 닉네임 (최대 12자):",cur)||"").trim()).slice(0,12); if(n) localStorage.setItem("cd_nick",n); return nickOr(); }
...
function boardRows(arr){ if(!arr) return `<p class="hbdim" style="padding:6px 0">불러오기 실패 — Apps Script 재배포(doGet) 확인</p>`; if(!arr.length) return `<p class="hbdim" style="padding:6px 0">아직 기록 없음 — 첫 주자가 되세요!</p>`;
  return arr.map((r,i)=>`<div class="hrow"><span><b style="color:${i<3?'var(--gold)':'#cdd6f5'}">${i+1}. ${r.nick}</b></span><span class="hname">${(r.score||0).toLocaleString()} <span class="hbdim">안테${r.ante||"-"}</span></span></div>`).join(""); }
function openBoard(){
  const sd=(S&&S.daily)?S.seed:dailySeed();
  document.getElementById("drawerBody").innerHTML=`<h3>🏆 리더보드</h3><p class="drawerSub">닉: <b>${nickOr()}</b> <span style="color:#9b6fd4;cursor:pointer;text-decoration:underline" onclick="setNick();openBoard()">변경</span></p><div id="bdDaily"><div class="seg">🗓 오늘 데일리 (#${sd})</div><p class="hbdim">불러오는 중…</p></div><div id="bdAll" style="margin-top:14px"><div class="seg">👑 전체 최고점</div><p class="hbdim">불러오는 중…</p></div>`;
  ...
  jsonp("daily", sd, d=>{ const el=document.getElementById("bdDaily"); if(el) el.innerHTML=`<div class="seg">🗓 오늘 데일리 (#${sd})</div>${boardRows(d)}`; });
  jsonp("alltime","", d=>{ const el=document.getElementById("bdAll"); if(el) el.innerHTML=`<div class="seg">👑 전체 최고점</div>${boardRows(d)}`; });
}
