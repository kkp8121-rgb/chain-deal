...
// 반환 {gained,base,runLen,bonus,mult,rawMult} — gained=최종 획득 점수(=score에 가산할 값), base=카드 기본점, runLen=체인 길이,
// bonus=체인 보너스(비연결 시 0), mult=캡 적용 직후(잭팟 chainMul/연결세 이전) 체인 배율(비연결 시 0), rawMult=mult의
// 클램프 直前 원값(비연결 시 0) — G1 mult-cap 게이트(tools/gate.cjs)가 캡(25, 닻 보스는 3)이 실제로 적용되는지
// (mult===min(rawMult,cap))를 검증하는 유일한 소비처(게임 로직은 mult만 사용, rawMult는 계측 전용). juicePlace 연출은
// base/runLen/bonus만 소비(호출부 책임 — main.cjs).
function scoreCard(row, card, left, ctx){
  ...
  let gained=base, runLen=1, bonus=0, mult=0, rawMult=0;
  ...
  if(left && ctx.connect(card,left) && !ctx.climbSealed(card,left) && !(ctx.boss('frost') && row.length<=2)){   // 냉각: 줄 첫 2장 연결 무효 / 내리막: 오름 ±1 봉인
    ...
    rawMult=mult;   // ★G1(b) 게이트 계측용: 클램프 前 원값(tools/gate.cjs가 실제 적용된 cap이 25/닻3와 일치하는지 검증하는 유일한 소비처) — 게임 로직엔 미사용
  ...
  return { gained, base, runLen, bonus, mult, rawMult };
}
